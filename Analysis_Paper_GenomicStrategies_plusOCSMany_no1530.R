...
head(acc)
acc$GroupCat <- paste0(acc$strategy, acc$scenario, acc$AgeCat)
acc <-  acc[acc$GroupCat %in% c("SU55PTcak5", "SU55GTgenTest1", "SU51GTgenTest1"),]
head(acc)
head(accOCS)

#aggregate by generations
...
accOCSa <- accOCSa[c("Cor", "Strategy", "Degree", "Rep")]
head(accOCSa)
colnames(accOCSa)[3] <- "Scenario"
...
accCompare <- accCompare[accCompare$Group %in% c("SU55PT","SU55GT","SU51GT","OCS15","OCS30","OCS45","OCS50","OCS55","OCS60","OCS75"),]
accCompare$Group <- factor(accCompare$Group, level=c("SU55PT","SU55GT","SU51GT","OCS15","OCS30","OCS45","OCS50","OCS55","OCS60","OCS75"))
...
TGVsOCS <- read.csv("~/Documents/Projects/inProgress/GenomicStrategies_SireUSe/TGVsAll_OCS_11022019.csv", sep=" ")
TGVsOCS1 <- read.csv("~/Documents/PhD/Projects/inProgress/GenomicStrategies_SireUse/Results//TGVsAll_OCS_20032019.csv", sep=" ")
TGVsOCS1 <- read.csv("~/Documents/Projects/inProgress/GenomicStrategies_SireUSe/TGVsAll_OCS_20032019.csv", sep=" ")
...
#TGVsAll <- rbind(TGVsAll, TGVsAllx)
...
table(TGVsAll$strategy, TGVsAll$scenario, TGVsAll$Rep)
library(plyr)
TGVsAll$scenario <- revalue(TGVsAll$scenario, c("Class" = "PT", "GenSLO" = "GT-PT", "OtherCowsGen" = "GT-C", "BmGen" = "GT-BD", "Gen" = "GT",
                                                "15"="15", "30"="30", "50" = "50", "55"="55", "45"="45", "60"="60", "75"="75"))


#add genetic and genic variance (standardised)
TGVsAll1 <- data.frame()
for (strategy in c("SU55", "SU51", "SU15", "SU52", "SU53", "SU54", "OCS")) {
  for (scenario in c("PT", "GT-PT", "GT-C", "GT-BD", "GT", 15, 30, 45, 50, 55, 60, 75)) {
    for (rep in 0:19) {
...
#imaš samo eno repliko
table(TGVsAll$Rep, TGVsAll$Strategy)
#TGVsAll <- TGVsAll[TGVsAll$Rep  %in% 0:9,]
#TGVsAll <- TGVsAll[TGVsAll$Generation %in% 40:51,]
...
for (scenario in c("PT","GT", 45, 50, 55, 60, 75)) {
  for (rep in 0:19) {
    #genetic gain
...
#OCS60 <- tgv60[tgv60$scenario %in% c("PT","GT", 15,

OCS60 <- tgv60[tgv60$PlotGroup %in% c("SU55PT","SU55GT", "SU51GT", "OCS45","OCS50","OCS55", "OCS60", "OCS75"),]
...
for (plotgroup in c("SU55PT","SU55GT", "SU51GT",    "OCS45","OCS50","OCS55", "OCS60", "OCS75")) {
...
EFF$Name <- factor(EFF$Name, level=c("SU55PT","SU55GT","SU51GT","OCS45","OCS50","OCS55","OCS60","OCS75"))
...
OCS60$scenario <- factor(OCS60$scenario, levels =c("PT", "GT", 45,50, 55,60,75))
MEAN60_OCS$Scenario <- factor(MEAN60_OCS$Scenario, levels =c("PT", "GT",45,50, 55,60,75))
MEAN60_OCS$Strategy <- factor(MEAN60_OCS$Strategy, levels =c("SU55", "SU51", "OCS"))
MEAN60_OCS_abs$Scenario <- factor(MEAN60_OCS_abs$Scenario, levels =c("PT", "GT", 45,50,55,60,75))
...
EFF$Scenario <- factor(EFF$Scenario, levels =c("PT", "GT", 45,50,55,60,75))
EFF$Strategy <- factor(EFF$Strategy, levels =c("SU55", "SU51", "SU15", "OCS"))

OCS60$Name <- paste0(OCS60$Strategy, OCS60$scenario)
OCS60$Name <- factor(OCS60$Name, level=c("SU55PT","SU55GT","OCS45","OCS50","OCS55","OCS60","OCS75"))
...
CLD = cld(marginal, 
          ...
          adjust  = "tukey") 
CLD
#by name = strategy+scenario - absolute
...
####################################################3
library(plyr)
#maxmin$scenario <- revalue(maxmin$scenario, c("Class" = "PT", "GenSLO" = "GT-PS", "OtherCowsGen" = "GT-C", "BmGen" = "GT-BD", "Gen" = "GT",
 #                                             "15"="15", "30"="30", "45"="45", "60"="60", "75"="75"))




maxminOS <- maxmin[maxmin$scenario %in% c(45, 50, 55, 60, 75, "PT", "GT"),]
...
#maxminPT <- maxminOS[maxminOS$PlotGroup %in% c("SU55PT", "SU55GT", "SU51GT", "SU52GT", "SU53GT", "SU54GT"),]
maxminPT <- maxminOS[maxminOS$PlotGroup %in% c("SU55PT", "SU55GT", "SU51GT"),]
maxminOCS <- maxminOS[maxminOS$scenario %in% c(45, 50, 55, 60, 75),]
...
TGVsStrategy <- TGVsAll[TGVsAll$PlotGroup %in% c("SU55PT", "SU55GT", "SU51GT",  "OCS45", "OCS50", "OCS55", "OCS60", "OCS75"),]
#TGVsStrategy <- TGVsAll[TGVsAll$PlotGroup %in% c("SU55PT", "SU55GT", "SU51GT", "SU52GT", "SU53GT", "SU54GT", "OCS15", "OCS30", "OCS45", "OCS50", "OCS55", "OCS60", "OCS75"),]
TGVsStrategy$strategy <- factor(TGVsStrategy$strategy, levels =c("SU55", "SU51","OCS"))
#TGVsStrategy$strategy <- factor(TGVsStrategy$strategy, levels =c("SU55", "SU51","SU52","SU53","SU54", "OCS"))
TGVsStrategy$scenario <- factor(TGVsStrategy$scenario, levels =c("PT","GT", 15, 30, 45, 50, 55, 60, 75))
TGVsStrategy$PlotGroup <- factor(TGVsStrategy$PlotGroup, level= c("SU55PT", "SU55GT", "SU51GT","OCS15", "OCS30", "OCS45", "OCS50", "OCS55", "OCS60", "OCS75"))
#TGVsStrategy$PlotGroup <- factor(TGVsStrategy$PlotGroup, level= c("SU55PT", "SU55GT", "SU51GT",  "SU52GT", "SU53GT", "SU54GT", "OCS15", "OCS30", "OCS45", "OCS50", "OCS55", "OCS60", "OCS75"))
...
# TGVsStrategy <- TGVsStrategy[order(TGVsStrategy$PlotGroup),]
maxminPT$PlotGroup <- factor(maxminPT$PlotGroup, level=c("SU55PT", "SU55GT","SU51GT"))
#maxminPT$PlotGroup <- factor(maxminPT$PlotGroup, level=c("SU55PT", "SU55GT","SU51GT","SU52GT", "SU53GT", "SU54GT", "SU15PT"))
maxminPT <- maxminPT[order(maxminPT$PlotGroup),]
maxminOCS$PlotGroup <- factor(maxminOCS$PlotGroup, level=c("OCS45","OCS50", "OCS55", "OCS60", "OCS75"))
#maxminOCS$PlotGroup <- factor(maxminOCS$PlotGroup, level=c("OCS15", "OCS30", "OCS45","OCS60", "OCS75"))
...
#TGVsStrategy <- TGVsStrategy[TGVsStrategy$Generation %in% 40:51,]
TGVsStrategy <- TGVsStrategy[TGVsStrategy$scenario %in% c(45, 50, 55, 60, 75, "PT", "GT"),]
#TGVsStrategy <- TGVsStrategy[TGVsStrategy$scenario %in% c(15, 30, 45, 60, 75, "PT", "GT"),]


#THIS IS VERY IMRORTANT!!!!
TGVsStrategy$group <- paste0(TGVsStrategy$PlotGroup, TGVsStrategy$Rep)
TGVsStrategy <- TGVsStrategy[(order(TGVsStrategy$group)),]
table(TGVsStrategy$group)
table(TGVsStrategy$PlotGroup)
TGVsStrategy$PlotGroup <- factor(TGVsStrategy$PlotGroup, level=c("SU55PT", "SU55GT","SU51GT", "OCS45","OCS50", "OCS55", "OCS60", "OCS75"))
...
ggplot(data = TGVsStrategy, aes(x=SDGenicSt, y=zMeanGenic, group=group, colour=PlotGroup)) +
 ...
 scale_linetype_manual("Breeding program",
                       breaks=c("OCS15", "OCS30", "OCS45","OCS60", "OCS75","SU55PT", "SU55GT", "SU51GT",  "SU52GT", "SU53GT", "SU54GT"),
                       values=c("F1", "longdash",  "dashed", "longdash",  "twodash", "solid", "solid", "solid","solid", "solid", "solid"),
                       labels=c(expression("OCS"["15"]), expression("OCS"["30"]), expression("OCS"["45"]),
                                ...
                                "5 sires/year, use 2 years, GT", "5 sires/year, use 3 years, GT", "5 sires/year, use 4 year, GT")) +
 scale_colour_manual("Breeding program",
                     breaks=c("OCS15", "OCS30", "OCS45", "OCS60", "OCS75","SU55PT", "SU55GT", "SU51GT",  "SU52GT", "SU53GT", "SU54GT"),
                     values=c("forestgreen", "orange", "purple", "darkblue",  "red3","blue", "pink", "black", "magenta", "grey50", "grey70"),
                     labels=c(expression("OCS"["15"]), expression("OCS"["30"]), expression("OCS"["45"]),
                              expression("OCS"["60"]), expression("OCS"["75"]) ,
                              "5 sires/year, use 5 years, PT", "5 sires/year, use 5 years, GT", "5 sires/year, use 1 year, GT",
                              "5 sires/year, use 2 years, GT", "5 sires/year, use 3 years, GT", "5 sires/year, use 4 year, GT")) +
...
ggplot(data = TGVsStrategy, aes(x=SDGenicSt, y=zMeanGenic, group=group, colour=PlotGroup)) +
 ...
 ylim(0,7) + coord_cartesian(xlim = c(1, 0.75)) + theme_bw() +
 scale_linetype_manual("Breeding program",
                       breaks=c("SU55PT", "SU55GT", "SU51GT","OCS15", "OCS30", "OCS45","OCS50","OCS55","OCS60", "OCS75"),
                       values=c("solid", "solid", "solid", "F1", "longdash",  "dashed", "longdash", "twodash","longdash", "twodash"),
                       labels=c( 
                                 "5 sires/year, use 5 years, PT", "5 sires/year, use 5 years, GT", "5 sires/year, use 1 year, GT"),
                       expression("OCS"["15"]), expression("OCS"["30"]), expression("OCS"["45"]),expression("OCS"["50"]),expression("OCS"["55"]),
                                                                                           expression("OCS"["60"]), expression("OCS"["75"])) +
 scale_colour_manual("Breeding program",
                     breaks=c("SU55PT", "SU55GT", "SU51GT", "OCS15", "OCS30", "OCS45","OCS50","OCS55", "OCS60", "OCS75"),
                     values=c("black", "grey50", "grey70","forestgreen", "orange2", "purple", "darkblue", "olivedrab2", "gold", "red3"),
                     labels=c(                         "5 sires/year, use 5 years, PT", "5 sires/year, use 5 years, GT", "5 sires/year, use 1 year, GT"),
                     expression("OCS"["15"]), expression("OCS"["30"]), expression("OCS"["45"]),expression("OCS"["50"]),expression("OCS"["55"]),
                              expression("OCS"["60"]), expression("OCS"["75"])) +
 guides(linetype=guide_legend(nrow=3,  label.position = "right", keyheight = unit(1, "cm"), keywidth = unit(3, "cm"), override.aes = list(alpha = 1, size=2))) +
 xlab("Genic standard deviation") + ylab("Genetic Mean") +
...
ggplot(data = TGVsStrategy, aes(x=SDGenicSt, y=zMeanGenic, group=group, linetype=PlotGroup, colour=PlotGroup)) +
  scale_x_reverse(sec.axis=sec_axis(trans=~ . - 1,
                                    name="Converted/Lost genic standard deviation")) +
  geom_line(size=0.2, alpha=0.2) +
  ylim(0,7) + coord_cartesian(xlim = c(1, 0.875)) + theme_bw() +
  scale_linetype_manual("Breeding program",
                        breaks=c("SU55PT", "SU55GT", "SU51GT","OCS45","OCS50","OCS55","OCS60", "OCS75"),
                        values=c( "solid", "solid", "solid", "dashed", "longdash", "twodash","dotdash", "F1"),
                        labels=c(
                          "5 sires/year, use 5 years, PT", "5 sires/year, use 5 years, GT", "5 sires/year, use 1 year, GT", 
  ...
  scale_colour_manual("Breeding program",
                      breaks=c("SU55PT", "SU55GT", "SU51GT", "OCS45","OCS50","OCS55", "OCS60", "OCS75"),
                      values=c("black", "grey50", "grey70",  "purple",  "red2","#00b300", "royalblue","darkorange"),
                      labels=c(
                        "5 sires/year, use 5 years, PT", "5 sires/year, use 5 years, GT", "5 sires/year, use 1 year, GT", 
                      expression("OCS"["45"]),expression("OCS"["50"]),expression("OCS"["55"]),
                               expression("OCS"["60"]), expression("OCS"["75"]))) +
  guides(linetype=guide_legend(nrow=4,  label.position = "right", keyheight = unit(0.5, "cm"), keywidth = unit(2, "cm"), 
  ...
  theme(axis.text=element_text(size=10), legend.position = "top",
        ...
        legend.text=element_text(size=10), legend.title=element_text(size=10), legend.box = "horizontal",
        plot.title = element_text(margin = margin(t = 0, r = 0, b = 40, l = 0), size=16, hjust=0.5),
...
ggplot(data = TGVsStrategy, aes(x=SDGenicSt, y=zMeanGenic, group=group, colour=PlotGroup)) +
 ...
 ylim(0,7) + coord_cartesian(xlim = c(1, 0.75)) + theme_bw() +
 scale_linetype_manual("Breeding program",
                       breaks=c("OCS15", "OCS30", "OCS45","OCS50","OCS55","OCS60", "OCS75","SU55PT", "SU55GT", "SU51GT",  "SU52GT", "SU53GT", "SU54GT"),
                       values=c("F1", "longdash",  "dashed", "longdash", "twodash","longdash", "twodash", "solid", "solid", "solid","solid", "solid", "solid"),
                       labels=c(expression("OCS"["15"]), expression("OCS"["30"]), expression("OCS"["45"]),expression("OCS"["50"]),expression("OCS"["55"]),
                                                                                           expression("OCS"["60"]), expression("OCS"["75"]) ,
                                "5 sires/year, use 5 years, PT", "5 sires/year, use 5 years, GT", "5 sires/year, use 1 year, GT",
                                "5 sires/year, use 2 years, GT", "5 sires/year, use 3 years, GT", "5 sires/year, use 4 year, GT")) +
 scale_colour_manual("Breeding program",
                     breaks=c("OCS15", "OCS30", "OCS45","OCS50","OCS55", "OCS60", "OCS75","SU55PT", "SU55GT", "SU51GT",  "SU52GT", "SU53GT", "SU54GT"),
                     values=c("forestgreen", "orange2", "purple", "darkblue", "olivedrab2", "gold", "red3","blue", "pink", "black", "magenta", "grey50", "grey70"),
                     labels=c(expression("OCS"["15"]), expression("OCS"["30"]), expression("OCS"["45"]),expression("OCS"["50"]),expression("OCS"["55"]),
                              expression("OCS"["60"]), expression("OCS"["75"]) ,
                              "5 sires/year, use 5 years, PT", "5 sires/year, use 5 years, GT", "5 sires/year, use 1 year, GT",
                              "5 sires/year, use 2 years, GT", "5 sires/year, use 3 years, GT", "5 sires/year, use 4 year, GT")) +
 guides(colour=guide_legend(nrow=3,  label.position = "right", keyheight = unit(1, "cm"), keywidth = unit(3, "cm"), override.aes = list(alpha = 1, size=2))) +
 xlab("Genic standard deviation") + ylab("Genetic Mean") +
 theme(axis.text=element_text(size=18), legend.position = "top",
       axis.title.x=element_text(size=18, vjust=-1),
       axis.title.y=element_text(size=18, vjust=2),
...
ggplot(data = TGVsAll[TGVsAll$Strategy %in% c("OCS") & TGVsAll$scenario != 15,], aes(x = Generation, y = zMean, colour=scenario, group=group, linetype=Strategy) ) + geom_line()+ 
  scale_colour_manual("Breeding program", 
                      values=c( "darkblue", "gold", "red3","blue", "magenta", "black")) 
  geom_line(data = TGVsAll[TGVsAll$Strategy %in% c("OCS"),], aes(x = Generation, y = zMean, colour=scenario, group=group, linetype=Strategy) ) + 
  scale_colour_manual("Breeding program", 
                      values=c( "black","orange2", "purple", "darkblue", "pink", "magenta", "grey50", "grey70")) 
ggplot(data = TGVsAll[TGVsAll$scenario==15,], aes(x = Generation, y = zMean, colour=Rep, group=Rep) ) + geom_line()
ggplot(data = TGVsAll[TGVsAll$scenario==30 & TGVsAll$Rep %in% 1:5,], aes(x = Generation, y = zMean, colour=Rep, group=Rep) ) + geom_line()
...
m$Scenario <- revalue(m$Scenario, c("Class" = "PT", "GenSLO" = "GT-PS", "OtherCowsGen" = "GT-C", "BmGen" = "GT-BD", "Gen" = "GT"))
m$Strategy <- revalue(m$Strategy, c("SU55" = "5 sires/year, use 5 years","SU51" = "5 sires/year, use 1 year","SU15" = "1 sire/year, use 5 years"))
m$Scenario <- factor(m$Scenario, levels =c("PT", "GT-PS", "GT-C", "GT-BD", "GT"))
...
VSE$Scenario <- revalue(VSE$Scenario, c("Class" = "PT", "GenSLO" = "GT-PS", "OtherCowsGen" = "GT-C", "BmGen" = "GT-BD", "Gen" = "GT"))
...
VSE$Scenario <- factor(VSE$Scenario, levels =c("PT", "GT-PS", "GT-C", "GT-BD", "GT"))
