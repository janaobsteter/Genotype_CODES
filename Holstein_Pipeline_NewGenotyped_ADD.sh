...
#script created temporary directory Genotipi_DATEDOWNLOAD with newly downloaded genotypes
...
#set paths
#1) path to the SNPchimpRepolocate
...
DOWNLOAD_PATH=~/Genotipi/Genotipi_DATA/Holstein
#3) path to the genotype directory - where you want to create you temp directory 
GEN_PATH=~/Genotipi/Genotipi_DATA/Holstein
...
#11)direcotry with CODES
CODE_DIR=/home/janao/Genotipi/Genotipi_CODES


#REQUIRED INPUT
#|||||||||||||||||||||||||||||||||||||||||||||||||
DATEDOWNLOAD=09082016 #date of downloading the file
FILE=Holstein0809* # downloaded file
#|||||||||||||||||||||||||||||||||||||||||||||||||
...
do
	...
	sed -i "s%PathToMap%$nmapfiles},$PWD/OUTPUT/PLINK_MERGED.map%g" PARAMFILE.txt
...
ls -d $PWD/*.ped > SNP800_ped.txt
ls -d $PWD/*.map > SNP800_map.txt
sed -n -i '1h;2,$H;${g;s/\n/,/g;p}' SNP800_ped.txt
sed -n -i '1h;2,$H;${g;s/\n/,/g;p}' SNP800_map.txt
