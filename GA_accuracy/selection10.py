...
class blupf90:
	def __init__(self, AlphaSimDir, codeDir, way=None, permEnv = False, varPE = 0):
		...
		if way == 'milk':
			...
			if self.permEnv: #tukaj ustvari permEnv za novorojene živali
				self.blupDatT = self.AlphaPed.loc[:, ['Indiv', 'phenoNormUnres1', 'cat', 'sex', 'age', 'active']]
				if os.path.isfile(self.AlphaDir + "PermanentEnv.txt"):
					permEnvFile = pd.read_csv("PermanentEnv.txt")#tole je nepotrebno
...
class AlphaRelate(object):
		def __init__(self, AlphaRelateDir, AlphaSimDir):
            ...
            try:
                shutil.copy(self.AlphaSimDir + "/IndForGeno.txt", self.AlphaRelateDir)
            except:
                pass
