...
TGVsAll_10K <- read.csv("~/Documents/PhD/Projects/inProgress/GenomicStrategies_SireUse/Results/TGVSALL_14082018.csv")
TGVsAll_1K <- read.csv("~/Documents/PhD/Projects/inProgress/GenomicStrategies_ReferenceSize/Results/TGVSALL_22082018.csv")
TGVsAll_10K <- TGVsAll_10K[TGVsAll_10K$Strategy=="SU55" & TGVsAll_10K$Rep %in% 0:2,]
TGVsAll_1K <- TGVsAll_1K[TGVsAll_1K$Strategy=="SU55" & TGVsAll_1K$Rep %in% 0:2,]
TGVsAll_10K$RefSize <- "10K"
TGVsAll_1K$RefSize <- "1K"
TGVsAll <- rbind(TGVsAll_1K, TGVsAll_10K)
...
Averages1 <- aggregate(TGVsAll$SDGenicSt ~ TGVsAll$RefSize + TGVsAll$Strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
Averages2 <- aggregate(TGVsAll$zMeanGenic ~ TGVsAll$RefSize + TGVsAll$Strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
Averages3 <- aggregate(TGVsAll$SDSt ~ TGVsAll$RefSize + TGVsAll$Strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
Averages4 <- aggregate(TGVsAll$zMean ~ TGVsAll$RefSize + TGVsAll$Strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
Averages5 <- aggregate(TGVsAll$SDGenic ~  TGVsAll$RefSize + TGVsAll$Strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
Averages6 <- aggregate(TGVsAll$sd ~ TGVsAll$RefSize + TGVsAll$Strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
Averages7 <- aggregate(TGVsAll$zMeanGenic_Genetic ~ TGVsAll$RefSize + TGVsAll$Strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
Averages8 <- aggregate(TGVsAll$Genic_Genetic_SDSt ~ TGVsAll$RefSize + TGVsAll$Strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
colnames(Averages1) <- c("RefSize", "strategy", "scenario", "Generation", "SDGenicSt") #stand
colnames(Averages2) <- c("RefSize","strategy","scenario", "Generation", "MeanGenic") #stand
colnames(Averages3) <- c("RefSize","strategy","scenario", "Generation", "SDGenetic") #stand
colnames(Averages4) <- c("RefSize","strategy","scenario", "Generation", "MeanGenetic") #standard
colnames(Averages5) <- c("RefSize","strategy","scenario", "Generation", "SDGenic_noSt")
colnames(Averages6) <- c("RefSize","strategy","scenario", "Generation", "SDGenetic_noSt")
colnames(Averages7) <- c("RefSize","strategy","scenario", "Generation", "MeanGenicGenetic") #standardizirana
colnames(Averages8) <- c("RefSize","strategy","scenario", "Generation", "SDGenic_Genetic") #standardizirana
Averages <- merge(Averages1, Averages2, by=c("RefSize", "strategy","scenario", "Generation"))
Averages <- merge(Averages, Averages3, by=c( "RefSize","strategy","scenario", "Generation"))
Averages <- merge(Averages, Averages4, by=c( "RefSize","strategy","scenario", "Generation"))
Averages <- merge(Averages, Averages5, by=c( "RefSize","strategy","scenario", "Generation"))
Averages <- merge(Averages, Averages6, by=c( "RefSize","strategy","scenario", "Generation"))
Averages <- merge(Averages, Averages7, by=c( "RefSize","strategy","scenario", "Generation"))
Averages <- merge(Averages, Averages8, by=c( "RefSize","strategy","scenario", "Generation"))


write.csv(Averages[Averages$Generation==60,], "~/Documents/PhD/Projects/inProgress/GenomicStrategies_ReferenceSize//Results//Averages_SU55_22082018.csv", quote=FALSE)
#AveragesA <- Averages
...
for (strategy in c("SU55", "SU15", "SU51")) {
  for (refsize in c("1K", "10K")) {
  #strategy <- "SU55"
    TGVstrategy <- TGVsAll[TGVsAll$Strategy==strategy & TGVsAll$RefSize==refsize,]
    TGVstrategy$Group <- paste0(TGVstrategy$scenario, TGVstrategy$Rep)
    TGVstrategy$Group <- as.factor(TGVstrategy$Group)
    ...
    plotList[[number]] <- ggplot(data = TGVstrategy, aes(x=SDGenicSt, y=zMeanGenic, group=Group, colour=scenario, linetype=scenario)) + 
      ...
      geom_line(aes(linetype=scenario), size=0.5, alpha=0.8) + ggtitle(paste0(unique(TGVstrategy$RefSize), "_", unique(TGVstrategy$strategy))) + 
      xlab("Generation") + ylab("True genetic value")  + ylim(0,7) +coord_cartesian(xlim = c(1, 0.9)) +
      ...
      theme(axis.text=element_text(size=20), legend.position = "left", 
            axis.title=element_text(size=20,face="bold"), legend.text=element_text(size=20), legend.title=element_text(face="bold", size=20)) +
...
MeanAverage <- aggregate(TGVsAll$zMean ~ TGVsAll$RefSize +  TGVsAll$strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
colnames(MeanAverage) <- c("RefSize", "Strategy", "scenario", "Generation", "MeanTGV")
...
MeanAverageSD <- aggregate(TGVsAll$SDSt ~  TGVsAll$RefSize + TGVsAll$strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
MeanAverageSDGenic <- aggregate(TGVsAll$SDGenicSt ~  TGVsAll$RefSize + TGVsAll$strategy + TGVsAll$scenario + TGVsAll$Generation, FUN="mean")
colnames(MeanAverageSD) <- c("RefSize", "Strategy", "scenario", "Generation", "SdTGV")
colnames(MeanAverageSDGenic) <- c("RefSize", "Strategy", "scenario", "Generation", "SdGenic")
...
m1 <- lm(MeanTGV~Scenario,data=Mean601[Mean601$Strategy=="SU55",])
m1.grid <- ref_grid(m1)
anova(m1)
m1S <- lsmeans(m1.grid, "Scenario")
contrast(m1.grid, method="pairwise")
...
MeanAverage$Group <- paste0(MeanAverage$RefSize, MeanAverage$scenario)
ggplot() + 
  xlab("Generation") + ylab("True genetic value")  + 
  scale_linetype_manual("RefSize", breaks = c("1K", "10K"), 
  ...
  geom_line(data = MeanAverage, aes(x=Generation, y=MeanTGV, group=Group, colour=scenario, linetype=RefSize), size=1.2) + 
...
s <- TGVsAll[TGVsAll$Strategy=="SU51",]
s$Group <- paste0(s$scenario, s$Rep)
ggplot(data=s, aes(x=Generation, y=zMean, group=Group, colour=scenario)) + geom_path()
ggplot(data=s, aes(x=Generation, y=SDGenicSt, group=Group, colour=scenario)) + geom_path()
...
write.csv(SD, "~/Documents/PhD/Projects/inProgress/GenomicStrategies_SireUse/Results//StandardDeviation_GeneticGain_gen60_14082018.csv", quote=FALSE)
