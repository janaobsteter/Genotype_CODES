  ...
  for (trait2 in c(2,3)) {
    for (scenarioHome in c("Gen")) {
	  ...
	  ped$GenerationSex <- paste0(ped$Generation, ped$Gender)
          ...
          sumPartHome <- summary(part, by="GenerationSex", subset = part$TGV$Group == "home")
	  sumPartHome <- sumPartHome$TGV
          sumPartHome$Population <- "home"
          sumPartHome$Rep <- rep
          sumPartHome$Import <- import
          sumPartHome$Scenario <- scenario
          sumPartHome$Cor <- ifelse(trait2 == 2, 0.9, 0.8)
          sumPartImport <- summary(part, by="GenerationSex", subset = part$TGV$Group == "import")
	  sumPartImport <- sumPartImport$TGV
...
write.table(partitions, paste0("Partition_Import_geneContributions_", date, "_GroupSex.csv"), quote=F, row.names=F)
