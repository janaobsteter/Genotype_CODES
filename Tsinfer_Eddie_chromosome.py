...
contigLength = pd.read_csv("ContigLengths.txt", header = None)

workingDir = os.getcwd() + "/"
chrDir = workingDir + "/Chr" + str(chromosome) 
os.system("mkdir " + chrDir)
os.chdir(chrDir)
...
# Create the chromosome vcf file
#os.system("bcftools view " + vcfFile + " --regions " + str(chromosome) + " > Chr" + str(chromosome) + ".vcf")
...
with tsinfer.SampleData(
    path=sampleFile, sequence_length=contigLength[0][chromosome-1]
) as samples:
...
print(
    "Sample file created for {} samples for chromosome {} ".format(samples.num_samples, str(chromosome))
...
print(
    "Inferred tree sequence `{}` chromosome {}: {} trees over {} Mb".format(
        "drone_ts", chromosome, drone_ts.num_trees, drone_ts.sequence_length / 1e6
    )
)
...
tree.draw(
    path=("tree_at_1Mb_chr" + str(chromosome) + ".svg"),
    height=700,
    width=1200,
...
tree.draw(
    path=("tree_at_1Mb_lineages_chr" + str(chromosome) + ".svg"),
    height=700,
    width=1200,
    node_labels=individual_for_node,
...
fstDF.to_csv("FST_chr" + str(chromosome) + ".csv", index=None)
...
tajimaDF.to_csv("TajimasD_chr" + str(chromosome) + ".csv", index=None)
...
divergenceDF.to_csv("Divergence_chr" + str(chromosome) + ".csv", index=None)
...
diversityDF.to_csv("Diversity_chr" + str(chromosome) + ".csv", index=None)



# Allele frequency spectrum 
np.savetxt("AFS_Alineage_chr" + str(chromosome) + ".csv", drone_ts.allele_frequency_spectrum([pops["A"]]))
np.savetxt("AFS_Mlineage_chr" + str(chromosome) + ".csv", drone_ts.allele_frequency_spectrum([pops["M"]]))
np.savetxt("AFS_Clineage_chr" + str(chromosome) + ".csv", drone_ts.allele_frequency_spectrum([pops["C"]]))
np.savetxt("AFS_Olineage_chr" + str(chromosome) + ".csv", drone_ts.allele_frequency_spectrum([pops["O"]]))
...
f2DF.to_csv("f2_chr" + str(chromosome) + ".csv", index=None)

# f4
f4 = drone_ts.f4([pops["A"], pops["C"], pops["M"], pops["O"]])
print("This is f4 statistics for chr " + str(chromosome) + ": ", str(f4))
...
gnn.to_csv("GNN_drones_chr" + str(chromosome) + ".csv", index=None)
...
grelDF.to_csv("GeneticRelatedness_chr" + str(chromosome) + ".csv", index=None)
...
meanDesc.to_csv("MeanDescendants_chr" + str(chromosome) + ".csv", index=None)


# Write the tables with mutations, nodes, sites, populations and individuals
drone_ts.dump_text(mutations=open("Mutations_chr" + str(chromosome) + ".txt", "w"), nodes = open("Nodes_chr" + str(chromosome) + ".txt", "w"), sites = open("Sites.txt", "w"), edges = open("Edges_chr" + str(chromosome) + ".txt", "w"), individuals = open("Individuals_chr" + str(chromosome) + ".txt", "w"), populations = open("Populations_chr" + str(chromosome) + ".txt", "w", encoding="utf-8"))
