...
colnames(giA) <- c("Line", "Scenario", "Strategy", "genInt")
...
for (strategy in c("SU55", "SU51", "SU15")) {
  for (scenario in c("Class", "GenSLO", "OtherCowsGen", "BmGen", "Gen")) {
    for (line in c("sireF", "sireM", "damF", "damM")) {
      for (rep in 0:19) {
        #genetic gain
        base <- gi$genInt[gi$scenario=="Class" & gi$strategy=="SU55" & gi$LINE==line & gi$Rep==rep]
        giLINE <- gi[gi$scenario==scenario & gi$strategy==strategy & gi$LINE==line & gi$Rep==rep,]
