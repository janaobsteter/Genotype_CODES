...
for rep in range(rounds):
    ...
    if not os.path.isdir(GAdir + "/Rep_" + str(rep)):
	os.makedirs(GAdir + "/Rep_" + str(rep))
    RepDir = GAdir + "/Rep_" + str(rep)
    os.chdir(RepDir)
    os.system("cp " + homeDir + "/Essentials/* .")
    os.system("cp " + homeDir + "/CodeDir/GA/* .")
    os.system("cp ../*ADF*mean.csv .")
    os.system("cp ../HerdNo.txt .")
    os.system("cp ../PedCows_HERDS* .")
    os.system("cp ../Ped*_cat.txt .")
   # os.system("/exports/cmvm/eddie/eb/groups/tier2_hickey_external/JanaO/anaconda2/bin/python GA_genotpingHerds2.py > GAherds.txt")
    #os.system("mkdir SimulatedData")
    os.system("cp " + WorkingDir + "/SimulatedData/Pedigree* SimulatedData/")
    os.system("cp " + WorkingDir + "/SimulatedData/TraitVarianceComponents.txt SimulatedData/")

    # ekstrahiraj rešitev
    print(os.getcwd())
    print("Trenutni direktorij!!!")
    ...
    snpmap = pd.read_csv(WorkingDir + '/SimulatedData/Chip1SnpInformation.txt',
                sep='\s+')[['SnpId', 'ChromId', 'SnpSeqIdOnChrom']]
    ...
    #zdj ustvari še .dat in .ped fajle
    blupNextGen = estimateBV(RepDir + "/", homeDir + "/CodeDir/", way="milk", sel="gen")
    ...
    pd.read_csv(WorkingDir + '/SimulatedData/Chip1SnpInformation.txt',
                sep='\s+')[['SnpId', 'ChromId', 'SnpSeqIdOnChrom']].to_csv('SnpMap.txt', index=None, sep=" ", header=None)
...
#    #vstavi ime za genotipsko datoteko
 #   os.system("sed 's/GENOTYPEFILE/GenoFileRandom.txt/g' renumf90_generic.par > renumf90.par")
    ...
    os.system(
        ...
        > GenoFile.txt''')  # obtain SNP genotypes of the last generation
    pd.read_csv(WorkingDir + '/SimulatedData/Chip1SnpInformation.txt',
                sep='\s+')[['SnpId', 'ChromId', 'SnpSeqIdOnChrom']].to_csv('SnpMap.txt', index=None, sep=" ", header=None)
