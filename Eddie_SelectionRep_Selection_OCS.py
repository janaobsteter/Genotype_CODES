...
print("Creating directory " + scenario + str(rep) + "OCS")
if not os.path.isdir(scenario + str(rep) + "OCS"):
    os.makedirs(scenario + str(rep) + "OCS")
SelectionDir = scenario + str(rep) + "OCS/"
...
for roundNo in range(21,41): #za vsak krog selekcije
    ...
    mate = AlphaMate(AlphaSimDir, AlphaSimDir)
    mate.prepareGender()
    mate.prepareCriterionFile()
    mate.prepareSpecFile(mate.countFemaleSel(), 5, mate.countFemaleSel(), 30)
    mate.runAlphaMate()
    ...
    ped.ped.loc[ped.ped.cat == "kandidati"] = "izl"
