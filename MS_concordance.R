...
write.table(MS_lab_68, "Intersect_lab.csv", sep=" ", quote=F, row.names = F)
write.table(MS_alel_68, "Intersect_imp.csv", sep=" ", quote=F, row.names = F)



###check intersect
Check <- matrix (nrow=nrow(MS_lab_68), ncol=12)
rownames(Check) <- MS_alel_68$ID #imena živali - oba fajla morata biti order by ID!!!

colnames(Check) <- c("BM1824","BM2113","ETH10","ETH225","ETH3","INRA23","SPS115","TGLA122","TGLA126","TGLA227","TGLA53","BM1818")
#

MS_col <- seq(from=1, to=24, by=2) #imena mikrosatelitov
