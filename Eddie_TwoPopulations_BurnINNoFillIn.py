...
#####################################################################################################
    ...
    par = pd.read_csv(os.getcwd() + "/SelectionParam_Class_LargePop.csv", header=None, names=["Keys", "Vals"])
    par.to_dict()
    selPar = defaultdict()
    ...
    for roundNo in range(1,21):  # za vsak krog selekcije
        if roundNo == 1:  # če je to prvi krog - nimaš še kategorij od prej, nimaš niti EBV-jev
            ...
            set_group_two(AlphaSimDir, "home", "import", int(selPar["stNBn"]) / 2)
            splitGenPed("PopulationSplit.txt")
            nastavi_cat('GenPed_EBVhome.txt', externalPedName = "ExternalPedigreehome", group=True, groupNumber=1, **selPar) #nastavi cat za prvo skupino
            nastavi_cat_TGV('GenPed_EBVimport.txt', externalPedName = "ExternalPedigreeimport", group=True, groupNumber=2, **selPar) #nastavi cat za drugo skupino
            joinExternalPeds(["ExternalPedigreehome", "ExternalPedigreeimport"])
            record_groups("PopulationSplit.txt")
            ...
            splitGenPed("PopulationSplit.txt")
            selekcija_total('GenPed_EBVhome.txt', externalPedName = "ExternalPedigreehome", group=True, groupNumber=1, **selPar)
            selekcija_total_TGV('GenPed_EBVimport.txt', externalPedName = "ExternalPedigreeimport", group=True, groupNumber=2, **selPar)
            joinExternalPeds(["ExternalPedigreehome", "ExternalPedigreeimport"])
            record_groups("PopulationSplit.txt")
