...
for (rep in 1) {
  for (import in c(100)) {
    for (scenario in c("GenGen")) {
      WorkingDir = paste0(homeDir, "/10K/",strategy, "_import/", scenario, rep, "_", import, "/SimulatedData/")
      UpDir = paste0(homeDir, "/10K/",strategy, "_import/", scenario, rep, "_", import, "/")
      ...
      ped <- ped[ped$Generation %in% 40:80,]
...
write.table(TGVsAll, paste0("TGVsAll_import80_", date, ".csv"), quote=FALSE, row.names=FALSE)
