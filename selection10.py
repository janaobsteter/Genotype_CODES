...
class pedigree(classPed):
    ...
    def izloci_poTGV(self, sex, stIzl, oldcat, prevGenDict, trait=1):
        izlRow = list(
            self.ped.loc[(self.ped.sex == sex) & (self.ped.Indiv.isin(prevGenDict[oldcat])), 'gvNormUnres' + str(trait)].sort_values(
                ascending=True)[:stIzl].index)  # katere izločiš
        if len(izlRow) < stIzl:
            print("Too little animals to choose from. <{} {}>".format("izloci po EBV", oldcat))
        else:
    ...
    def izloci_poTGV_age(self, sex, age, stIzl, oldcat, prevGenDict, trait=1):
        izlRow = list(self.ped.loc[(self.ped.age == age) & (self.ped.sex == sex) & (
            self.ped.Indiv.isin(prevGenDict[oldcat])), 'gvNormUnres' + str(trait)].sort_values(ascending=True)[
                      :stIzl].index)  # katere izločiš
        if len(izlRow) < stIzl:
    ...
    def izberi_poTGV_top(self, sex, st, oldcat, cat, prevGenDict, trait=1):
        selRow = list(
            self.ped.loc[(self.ped.sex == sex) & (self.ped.Indiv.isin(prevGenDict[oldcat])), 'gvNormUnres' + str(trait)].sort_values(
                ascending=False)[:st].index)  # katere izbereš
        if len(selRow) < st:
    ...
    def izberi_poTGV_top_catCurrent(self, sex, st, cat,
                                    newcat, trait = 1):  # to je bolj kot ne samo za prvi krog, ko nimaš slovarja od prejšnje generacije
        selRow = list(self.ped.loc[(self.ped.sex == sex) & (self.ped.cat == cat), 'gvNormUnres' + str(trait)].sort_values(ascending=False)[
                      :st].index)  # katere izbereš
        if len(selRow) < st:
    ...
    def izberi_poTGV_top_age_naive(self, sex, age, st,
                                   newcat, trait = 1):  # to je bolj kot ne samo za prvi krog, ko nimaš slovarja od prejšnje generacije
        selRow = list(
            self.ped.loc[(self.ped.sex == sex) & (self.ped.age == age) & (self.ped.cat == ""), 'gvNormUnres' + str(trait)].sort_values(
                ascending=False)[:st].index)  # katere izbereš
        if len(selRow) < st:
    ...
    def izberi_poTGV_top_age(self, sex, age, st, oldcat, cat, prevGenDict, trait = 1):
        selRow = list(self.ped.loc[(self.ped.age == age) & (self.ped.sex == sex) & (
            self.ped.Indiv.isin(prevGenDict[oldcat])), 'gvNormUnres' + str(trait)].sort_values(ascending=False)[:st].index)  # katere izbereš
        if len(selRow) < st:
    ...
    def ind_poTGV_top(self, sex, st, cat, prevGenDict, trait=1):
        selRow = list(self.ped.loc[(self.ped.sex == sex) & (self.ped.Indiv.isin(prevGenDict[cat])), 'gvNormUnres' + str(trait)].sort_values(
            ascending=False)[:st].index)  # katere izbereš
        if len(selRow) < st:
    ...
    def izberi_poTGV_OdDo(self, sex, start, stop, oldcat, cat, prevGenDict, trait = 1):
        selRow = list(
            self.ped.loc[(self.ped.sex == sex) & (self.ped.Indiv.isin(prevGenDict[oldcat])), 'gvNormUnres' + str(trait)].sort_values(
                ascending=False)[start:stop].index)  # katere izbereš
        if len(selRow) < (stop - start):
    ...
    def izberi_poTGV_OdDo_age_naive(self, sex, age, start, stop, cat, trait = 1):
        selRow = list(
            self.ped.loc[(self.ped.sex == sex) & (self.ped.age == age) & (self.ped.cat == ""), 'gvNormUnres' + str(trait)].sort_values(
                ascending=False)[start:stop].index)  # katere izbereš
        if len(selRow) < (stop - start):
...
class blupf90:
    def __init__(self, AlphaSimDir, codeDir, way=None, permEnv = False, varPE = 0, herd = False, varH = 0, trait = 1):
        ...
        if way == 'milk':
            ...
            if self.permEnv:
                self.blupDatT = self.AlphaPed.loc[:, ['Indiv', 'phenoNormUnres' + str(self.trait), 'cat', 'sex', 'age', 'active', 'herdYear']]
                ...
                self.blupDatT = self.AlphaPed.loc[:,
                                ['Indiv', 'phenoNormUnres' + str(self.trait), 'cat', 'sex', 'age', 'active']]
        ...
        if way == 'burnin_milk':
            ...
            self.animals = len(self.AlphaPed)
            if not herd:
                self.blupDatT = self.AlphaPed.loc[:, ['Indiv', 'phenoNormUnres' + str(self.trait), 'sex']]
            if herd:
                self.blupDatT = self.AlphaPed.loc[:, ['Indiv', 'phenoNormUnres' + str(self.trait), 'sex']]
            self.blupPed = self.AlphaPed.loc[:, ['Indiv', 'Father', 'Mother']]
    ...
    def deletePhenotype_cat(self, listUnphenotyped):
        for i in listUnphenotyped:
            self.blupDatT.loc[self.blupDatT.cat == i, 'phenoNormUnres' + str(self.trait)] = 0

    def deletePhenotype_sexage(self, sex, age):
        self.blupDatT.loc[(self.blupDatT.sex == sex) & (self.blupDatT.age == age), 'phenoNormUnres' + str(self.trait)] = 0

    def deletePhenotype_sex(self, sex):
        self.blupDatT.loc[self.blupDatT.sex == sex, 'phenoNormUnres' + str(self.trait)] = 0
    ...
    def makeDat_sex(self, sex):  # THIS IS not for the repeatability model
        ...
        self.blupDatT[['Indiv', 'phenoNormUnres' + str(self.trait), 'sex', 'Mean']].to_csv(self.AlphaSimDir + 'Blupf90_trait' +
                                                                                           str(self.trait) + '.dat', header=None, index=False, sep=" ")

    def makeDat_sex_herd(self, sex):  # THIS IS not for the repeatability model
        # first remove animals that do not have phenotype (chosen sex)
        self.blupDatT = self.blupDatT.loc[self.blupDatT.sex == sex]
        self.blupDatT.loc[:, "Mean"] = 1
        self.blupDatT[['Indiv', 'phenoNormUnres' + str(self.trait), 'sex', 'herdYear', 'Mean']].to_csv(self.AlphaSimDir + 'Blupf90_trait' +
                                                                                           str(self.trait) + '.dat',
                                                                                      header=None,
                                                                                      index=False, sep=" ")
    ...
    def makeDat_removePhen_milk(self):
        ...
        blupDatNew = self.blupDatT.loc[
            (self.blupDatT.active == 1) & ((self.blupDatT.cat == 'k') | (self.blupDatT.cat == 'bm')
                                           | (self.blupDatT.cat == 'pBM')), ['Indiv', 'phenoNormUnres'+str(self.trait), 'sex']]
        ...
        if os.path.isfile(self.AlphaSimDir + 'Blupf90_trait' + str(self.trait) + '.dat'):
            print("Writing old plus new")
            blupDatOld = pd.read_csv('Blupf90_trait' + str(self.trait) + '.dat', sep=" ", names=['Indiv', 'phenoNormUnres' + str(self.trait),
                                                                    'sex',
                                                                    'Mean'])  # to je dat iz prejšnjega kroga selekcija
            print("This is the head of the data file")
            print('Indiv' + 'phenoNormUnres' + str(self.trait) + 'sex'+ 'Mean')
            print(blupDatNew.head())
            dat = pd.concat([blupDatOld, blupDatNew])
            dat.Mean = 1
            print("This is the head of the OLD data file")
            print(dat.head())

            dat.to_csv(self.AlphaSimDir + 'Blupf90_trait' + str(self.trait) + '.dat', header=None, index=False,
                                                       sep=" ")  # dodaj fenotip
        else:
            print("Writing new")
            blupDatNew.to_csv(self.AlphaSimDir + 'Blupf90_trait' + str(self.trait) + '.dat', header=None, index=False, sep=" ")
    ...
    def makeDat_removePhen_milk_repeatedPhenotype_herd(self, varE, repeats):
        ...
        blupDatUnpheno = self.blupDatT.loc[(self.blupDatT.active == 1) & (~self.blupDatT.Indiv.isin(blupDatNew.Indiv)),
                                           ['Indiv', 'phenoNormUnres' + str(self.trait), 'sex', 'herdYear', 'permEnv']]
        traitName = 'phenoNormUnres' + str(self.trait)
        blupDatUnpheno[traitName] = '0'
        ...
        if os.path.isfile(self.AlphaSimDir + 'Blupf90_trait' + str(self.trait) + '.dat'):
            blupDatOld = pd.read_csv('Blupf90_trait' + str(self.trait) + '.dat', sep=" ", names=['herdYear', 'phenoNormUnres' + str(self.trait),
                                                                    'Indiv', 'sex',
                                                                    'Mean'], low_memory=False)  # to je dat iz prejšnjega kroga selekcija
            dat = pd.concat([blupDatOld, phenoNew])
            dat.Mean = 1
            dat.to_csv(self.AlphaSimDir + 'Blupf90_trait' + str(self.trait) + '.dat', header=None, index=False,
                                                     sep=" ", columns = ['herdYear', 'phenoNormUnres' + str(self.trait), 'Indiv', 'sex', 'Mean'])
        else:
            phenoNew.to_csv(self.AlphaSimDir + 'Blupf90_trait' + str(self.trait) + '.dat', header=None, index=False, sep=" ",
                            columns = ['herdYear', 'phenoNormUnres' + str(self.trait), 'Indiv', 'sex', 'Mean'])
    ...
    def makeDat_removePhen_cat(self, listUnphenotyped):
        ...
        if os.path.isfile(self.AlphaSimDir + 'Blupf90_trait' + str(self.trait) + '.dat'):
            blupDatOld = pd.read_csv('Blupf90_trait' + str(self.trait) + '.dat', sep=" ", names=['Indiv', 'phenoNormUnres' + str(self.trait), 'sex', 'Mean'])
            blupDatNew = blupDatT[['Indiv', 'phenoNormUnres' + str(self.trait), 'sex']]
            blupDatNew.loc[:, "Mean"] = 1
            dat = pd.concat([blupDatOld, blupDatNew])
            dat.Mean = 1
            dat.to_csv(self.AlphaSimDir + 'Blupf90_trait' + str(self.trait) + '.dat', header=None, index=False,
            ...
            self.blupDatT.loc[:, "Mean"] = 1
            self.blupDatT[['Indiv', 'phenoNormUnres' + str(self.trait), 'sex', 'Mean']].to_csv(self.AlphaSimDir + 'Blupf90_trait' + str(self.trait) + '.dat',
                                                                              header=None,
                                                                              index=False, sep=" ")
    ...
    def prepareSelPed(self, multipleTraits = None):
        blupSol = pd.read_csv(self.AlphaSimDir + '/renumbered_Solutions_' + str(self.gen), header=None,
                              sep='\s+', names=['renID', 'Indiv', 'EBV'])
        if not multipleTraits:
            AlphaSelPed = self.AlphaPed.loc[:, ['Generation', 'Indiv', 'Father', 'Mother', 'gvNormUnres' + self.trait]]
        elif multipleTraits:
            traitsCols = ['gvNormUnres' + str(x) for x in multipleTraits]
            AlphaSelPedInfo = self.AlphaPed.loc[:, ['Generation', 'Indiv', 'Father', 'Mother']]
            AlphaSelPedTraits = self.AlphaPed.loc[:, traitsCols]
    ...
    def  prepareSelPed_group(self, splitfile, multipleTraits = None):
        ...
        :param traitDict: Dictionary containing the traits for each group
        :return: writes GenPed_EBV_group.txt for each group
        """
        print("Creating group GenPed_EBVs")
        traitsCols = ['gvNormUnres' + str(x) for x in multipleTraits]
    ...
    def popSplitDat(self, datFile, splitFile):
        ...
        for group in set(DAT.Group):
            DAT[DAT.Group == group][dat.columns].to_csv("Blupf90_trait" + str(self.trait) + "_" + group + ".dat", sep=" ", header=None, index=False)
...
class repeatedPhenotypes(object):
    ...
    def inds_to_keep(self, list, trait):
        self.selectedPed = self.PED[self.PED.Indiv.isin(list)][['Indiv', 'sex', 'gvNormUnres' + str(trait), 'herd', 'herdYear', 'permEnv']]

    def simulatePhenotype(self, varE, repeats, trait = 1):
        ...
        for row in range(len(self.selectedPed)):
            ...
            new = OrderedDict({"Indiv": int(indiv), 'phenoNormUnres' + str(self.trait):
...
class accuracies:
    def __init__(self, AlphaSimDir, group = "", trait = 1):
        self.AlphaSimDir = AlphaSimDir
    ...
    def saveAcc(self):
        ...
        corCatX = pd.DataFrame(
            {'AgeCat': list((pdPed.groupby('AgeCat')[['gvNormUnres' + str(self.trait), 'EBV']].corr().ix[0::2, 'EBV'].index.levels)[0]),
             'Cor': list(pdPed.groupby('AgeCat')[['gvNormUnres' + str(self.trait), 'EBV']].corr().ix[0::2, 'EBV']),
             'Gen': gen})
...
class AlphaSim_OutputFile:
    def __init__(self, AlphaSimDir):
        self.AlphaSimDir = AlphaSimDir
        os.system("grep Var " + self.AlphaSimDir + "/SimulatedData/TraitVarianceComponents.txt  | grep -v Variance > " + self.AlphaSimDir + "/SimulatedData/OnlyVariance.txt")
        self.TraitVariance = pd.read_csv(self.AlphaSimDir + '/SimulatedData/OnlyVariance.txt', sep='\s+', header=None)

    def getAddVar(self, trait):
        return float(list(self.TraitVariance[1][self.TraitVariance[0] == "VarA"])[trait-1])

    def getResVar(self, trait):
        return float(list(self.TraitVariance[1][self.TraitVariance[0] == "VarE"])[trait-1])

    def getGenVar(self, trait):
        return float(list(self.TraitVariance[1][self.TraitVariance[0] == "VarG"])[trait-1])

    def getDomVar(self, trait):
        return float(list(self.TraitVariance[1][self.TraitVariance[0] == "VarD"])[trait-1])
...
#################
def selekcija_total_TGV(pedFile, externalPedName = "ExternalPedigree", group=False, groupNumber=None, groupName = "", noGroups=1, trait = 1, **kwargs):
    print kwargs
    ped = pedigree(pedFile)
    ...
    if max(ped.gen) == 1:  # ČE SAMO ENA GENERACIJA V PEDIGREJU! - to zdj ne delaš več tako
        ...
        # določi spol
        # ped.set_sex_AlphaSim(kwargs.get('AlphaSimDir'))
        ped.izberi_poTGV_top_catCurrent("F", int(kwargs.get('potomciNPn')), 'nr', 'potomciNP', trait = trait)
        ped.izberi_poTGV_top_catCurrent("M", int(kwargs.get('potomciNPn')), 'nr', 'potomciNP', trait = trait)
    ...
    ped.izberi_poTGV_top("F", kwargs.get('telFn'), "nr", "telF",
                         categories, trait = trait)  # izberi NB ženske, ki preživijo in postanejo telice
    ped.izloci_poTGV("F", izlF, "nr", categories, trait = trait)  # cull females (lowest on EBV) tukaj jih izloči, funkcija v modulu

    # age 1 - pri enem letu osemeni določeno število telic (% določen zgoraj), druge izloči
    if 'telF' in categories.keys():
        ped.izberi_poTGV_top("F", kwargs.get('ptn'), 'telF', 'pt', categories, trait = trait)  # plemenske telice
        ped.izloci_poTGV("F", (len(categories['telF']) - kwargs.get('ptn')), 'telF', categories, trait = trait)  # preostale izloči
    ...
    if ('k' in categories.keys()) and ((1 + kwargs.get('bmOdbira')) in ped.age()):
        ped.izberi_poTGV_top_age("F", kwargs.get('bmOdbira') + 1, int(kwargs.get('bmn') / kwargs.get('bmUp')), 'k',
                                 'pBM',
                                 categories, trait = trait)  # izberi BM, ki jih osemeniš (plemenske BM = pBM) iz krav po 2. laktaciji
        ped.set_active_cat('pBM', 1, categories)
    # ostale BM prestavi naprej - BM po 1. do izločitvene laktacije
    if 'pBM' in categories.keys():
        for i in range((1 + kwargs.get('bmOdbira') + 1), (
    ...
    if 'vhlevljeni' in categories.keys():  # če imaš vhlevljene bike (samo v progenem testu)
        ...
        if kwargs.get("genTest_gpb") and 'vhlevljeni' in [i[0] for i in kwargs[
            'genotyped']]:  # če se greš genomsko shemo, jih takoj pogenotipiziraj in prestavi v gpb (5)
            ped.izberi_poTGV_top("M", kwargs.get('genpbn'), "vhlevljeni", "gpb", categories, trait = trait)  # odberi gpb
            ped.izberi_poTGV_OdDo("M", kwargs.get('genpbn'), (kwargs.get('genpbn') + kwargs.get('pripust1n')),
                                  'vhlevljeni', 'pripust1',
                                  categories, trait = trait)  # preostali genomsko testirani gredo v pripust
            ped.izloci_poTGV("M", (kwargs.get("vhlevljenin") - (kwargs.get('genpbn') + kwargs.get('pripust1n'))),
                             "vhlevljeni", categories, trait = trait)
        #############################
        else:  # drugače 8 v mlade
            ped.izberi_poTGV_top("M", kwargs.get('mladin'), "vhlevljeni", "mladi", categories, trait = trait)  # odberi mlade
            ped.izberi_poTGV_OdDo("M", kwargs.get('mladin'), kwargs.get('vhlevljenin'), "vhlevljeni", "pripust1",
                                  categories, trait = trait)  # preostali vhlevljeni gredo v pripust
    ...
    if 'mladi' in categories.keys():
        ...
        if kwargs.get("genTest_gpb") and 'mladi' in [i[0] for i in kwargs[
            'genotyped']]:  # če se greš genomsko shemo, jih takoj pogenotipiziraj in prestavi v gpb (5)
            ped.izberi_poTGV_top("M", kwargs.get('genpbn'), "mladi", "gpb", categories, trait = trait)
            ped.izloci_poTGV("M", (kwargs.get("mladin") - kwargs.get("genpbn")), "mladi", categories, trait = trait)
        ###########################
        else:
            ped.set_cat_old('mladi', 'cak', categories)  # mlade prestavi v cakajoce in jih izloci iz populacije
            ped.set_active_cat('mladi', 2, categories)
    ...
    if 'cak' in categories.keys():
        ...
        if kwargs.get("genTest_gpb") and 'cak' in [i[0] for i in kwargs[
            'genotyped']]:  # če se greš genomsko shemo, jih takoj pogenotipiziraj in prestavi v gpb (5)
            ped.izberi_poTGV_top("M", kwargs.get('genpbn') * kwargs.get("cak"), "cak", "gpb", categories, trait = trait)
            ped.izloci_poTGV("M", ((kwargs.get("mladin") - kwargs.get('genpbn')) * kwargs.get("cak")), "cak",
                             categories, trait = trait)
        ##########################
        else:
    ...
    if ('cak' in categories.keys()) and (
            ...
            kwargs.get("EBV") or kwargs.get("gpb_pb") or kwargs.get("genTest_mladi")) and 'cak' not in [i[0] for i in
                                                                                                        kwargs[
                                                                                                            'genotyped']]:  # +2 - eno leto so teleta, eno leto mladi biki, če imaš genomsko, gredo že pred do gpb
        ped.izberi_poTGV_top_age("M", (kwargs.get('cak') + 2), kwargs.get('pbn'), 'cak', 'pb', categories, trait = trait)
        ped.set_active_cat('cak', 2,
                           categories)  # tukaj moraš to nastaviti, zato ker fja izberi avtomatsko nastavi na active=1, vsi cakajoci so izloceni
        ped.izloci_poTGV_age("M", (kwargs.get('cak') + 2), (kwargs.get('mladin') - kwargs.get('pbn')), 'cak',
                             categories, trait = trait)  # TUKAJ MORA BITI ŠE STAROST!!!!!!!!!!!
    ...
    if 'genTest' in categories.keys():  # če imaš genomsko testirane bike
        if kwargs.get('genTest_mladi'):
            ...
            ped.izberi_poTGV_top("M", kwargs.get('mladin'), 'genTest', 'mladi',
                                 categories, trait = trait)  # naslednji najboljši gredo v test
            ped.izberi_poTGV_OdDo("M", kwargs.get('mladin'), kwargs.get('vhlevljenin'), 'genTest', 'pripust1',
                                  categories, trait = trait)  # preostali genomsko testirani gredo v pripust
            ped.izloci_poTGV("M", (kwargs.get('potomciNPn') - kwargs.get('vhlevljenin')), "genTest", categories, trait = trait)

        if kwargs.get('genTest_gpb'):
            ped.izberi_poTGV_top("M", kwargs.get('genpbn'), "genTest", "gpb",
                                 categories, trait = trait)  # odberi genomsko testirane bike za AI
...
def nastavi_cat_TGV(PedFile, externalPedName = "ExternalPedigree.txt", group=False, groupNumber=None, noGroups=1, trait = 1, **kwargs):
    ped = pedigree(PedFile)
    ped.compute_age()
    ...
    if kwargs.get('EBV'):
        # age 0
        ped.izberi_poTGV_top_age_naive('M', 0, kwargs.get('vhlevljenin'), 'vhlevljeni', trait = trait)  # določi vhlevljene
        # age1
        # določi mlade
        ped.izberi_poTGV_top_age_naive('M', 1, kwargs.get('mladin'), 'mladi', trait = trait)
        # določi pripust - 1. leto
        ped.izberi_poTGV_OdDo_age_naive('M', 1, kwargs.get('mladin'), kwargs.get('vhlevljenin'), 'pripust1', trait = trait)
        # age2,3,4
        for i in range(2, 2 + kwargs.get('cak')):  # leta, ko so cakajoci
            ped.izberi_poTGV_top_age_naive('M', i, kwargs.get('mladin'), 'cak', trait = trait)

        # od 1-2 leta v pripustu
        pripust2 = int(round(kwargs.get('pripust2n') * len(ped.catCurrent_indiv('pripust1'))))
        ped.izberi_poTGV_OdDo_age_naive('M', 2, kwargs.get('mladin'), (kwargs.get('mladin') + pripust2), 'pripust2', trait = trait)

        # age 5 - 10: pb
        pbAge = range((2 + kwargs.get('cak')), (2 + kwargs.get('cak') + kwargs.get('pbUp'))) if (2 + kwargs.get(
            'cak') + kwargs.get('pbUp')) <= max(ped.gens()) else range((2 + kwargs.get('cak')), max(ped.gens()))
        for i in pbAge:
            ped.izberi_poTGV_top_age_naive('M', i, kwargs.get('pbn'), 'pb', trait = trait)

    if kwargs.get('gEBV'):
        ped.izberi_poTGV_top_age_naive('M', 0, kwargs.get('potomciNPn'), 'genTest', trait = trait)

        # določi pripust - 1. leto
        ped.izberi_poTGV_OdDo_age_naive('M', 1, kwargs.get('genpbn'), kwargs.get('vhlevljenin'),
                                        'pripust1', trait = trait)  # kateri niso odbrani po genomskih, gredo za pripust

        # od 1-2 leta v pripustu
        pripust2 = int(round(kwargs.get('pripust2n') * len(ped.catCurrent_indiv('pripust1'))))
        ped.izberi_poTGV_OdDo_age_naive('M', 2, kwargs.get('genpbn'), (kwargs.get('genpbn') + pripust2), 'pripust2', trait = trait)

        for i in range(1, (
                    2 + kwargs.get(
                    'cak'))):  # od enega leta pa do konca progenega testa so genomsko testirani, kasneje progeno
            ped.izberi_poTGV_top_age_naive('M', i, kwargs.get('genpbn'), 'gpb', trait = trait)  # odberi genomsko testirane bike za AI
        ...
        pbAge = range((2 + kwargs.get('cak')), (2 + kwargs.get('cak') + kwargs.get('pbUp'))) \
            if (2 + kwargs.get('cak') + kwargs.get('pbUp')) <= max(ped.gens()) else range((2 + kwargs.get('cak')),
                                                                                          max(ped.gens()))
        for i in pbAge:
            ped.izberi_poTGV_top_age_naive('M', i, kwargs.get('genpbn'), 'pb', trait = trait)

    # FEMALES
    # age 0
    # določi ženska teleta pod 12
    ped.izberi_poTGV_top_age_naive('F', 0, kwargs.get('telFnTotal'), 'telF', trait = trait)

    # age1
    # določi plemenske telice
    ped.izberi_poTGV_top_age_naive('F', 1, kwargs.get('ptn'), 'pt', trait = trait)

    # age2 #pri starosti 2 let so samo plemenske telice, nima tudi BM
    for i in range(2, (1 + kwargs.get('bmOdbira'))):
        ped.izberi_poTGV_top_age_naive('F', i, kwargs.get('ptn'), 'k', trait = trait)

    # age3,4,5 #tukaj imaš enako število krav - minus tisti del, ki postane BM
    # odberi plemenske bm najprej
    for i in range((1 + kwargs.get('bmOdbira')), (1 + kwargs.get('bmOdbira') + kwargs.get('bmUp'))):
        ped.izberi_poTGV_top_age_naive('F', i, int(kwargs.get('bmn') / kwargs.get('bmUp')), 'pBM', trait = trait)
    ...
    ped.izberi_poTGV_top_age_naive('F', (1 + kwargs.get('bmOdbira') + kwargs.get('bmUp')),
                                   int(kwargs.get('bmn') / kwargs.get('bmUp')), 'bm', trait = trait)
...
class AlphaSimSpec:
    def __init__(self, AlphaSimDir, CodeDir, type=""):
        self.genSpecFile = CodeDir + "/AlphaSimSpec" + type + ".txt"
        shutil.copy(self.genSpecFile, AlphaSimDir + "/AlphaSimSpec.txt")
