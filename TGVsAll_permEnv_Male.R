...
TGVsAll <- data.frame()
for (rep in c(0, 2)) {
  for (scenario in c(0.1, 0.2, 0.5, 1.0, 1.5, 1.8)) {
	    WorkingDir = paste0("/home/v1jobste/JanaO/10K/",strategy, "_permEnv/Gen", rep, "_11_",  sprintf("%.1f", scenario),  "/SimulatedData/")
...
write.table(TGVsAll, paste0("TGVsAll_permEnv_varE", strategy, "_", date, ".csv"), quote=FALSE, row.names=FALSE)
