...
class pedigree(classPed):
 ...
 #       gender = pd.read_table(AlphaSimDir + '/Gender_BURNIN.txt', sep='\s+')
    ...
    def izberi_poEBV_top_age_naive(self, sex, age, st, newcat): #to je bolj kot ne samo za prvi krog, ko nimaš slovarja od prejšnje generacije
        selRow = list(self.ped.loc[(self.ped.sex==sex) & (self.ped.age==age) & (self.ped.cat == ""), 'EBV'].sort_values(ascending=False)[:st].index) #katere izbereš
        if len(selRow) < st:
    ...
    def izberi_poEBV_OdDo_age_naive(self,  sex, age, start, stop,  cat):
        selRow = list(self.ped.loc[(self.ped.sex==sex) &(self.ped.age==age) & (self.ped.cat == ""), 'EBV'].sort_values(ascending=False)[start:stop].index) #katere izbereš
        if len(selRow) < (stop - start):
...
class blupf90:
    def __init__(self, AlphaSimDir, way=None, sel=None):
        ...
        #self.blupParamFile = AlphaSimDir + 'renumf90.par'
    ...
    def prepareSelPed(self):
        blupSol = pd.read_csv(self.AlphaSimDir + '/renumbered_Solutions_' + str(self.gen), skiprows=1, header=None,
                              sep='\s+', names=['renID', 'ID', 'leve', 'Solution'])
...
class accuracies:
    ...
    def saveAcc(self):
        ...
        EBV = pd.read_table(self.AlphaSimDir + 'renumbered_Solutions_' + str(gen), #header=None,
                            sep='\s+') #names = ['renID', 'ID', 'level', 'solution'])
...
def nastavi_cat (PedFile, **kwargs):
    ...
    if kwargs.get('EBV'):
        ...
        ped.izberi_poEBV_OdDo_age_naive('M', 2, kwargs.get('mladin'), (kwargs.get('mladin') + kwargs.get('pripust2n')), 'pripust2')
    ...
    if kwargs.get('gEBV'):
        ...
        #določi pripust - 1. leto
        ped.izberi_poEBV_OdDo_age_naive('M',1, kwargs.get('pbn'), kwargs.get('potomciNPn'), 'pripust1') #kateri niso odbrani po genomskih, gredo za pripust
        
        #od 1-2 leta v pripustu
        ped.izberi_poEBV_OdDo_age_naive('M',2, kwargs.get('pbn'), (kwargs.get('pbn') + kwargs.get('pripust2n')), 'pripust2')
    ...
    #age 6
    #izberi odslužene bm
    ped.izberi_poEBV_top_age_naive('F',(1 + kwargs.get('bmOdbira') + kwargs.get('bmUp')), int(kwargs.get('bmn') / kwargs.get('bmUp')), 'bm')
