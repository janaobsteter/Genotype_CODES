...
TGVsAll <- data.frame()
for (rep in 0:4) {
  for (repeats in c(1, 2, 5, 8, 9, 11)) {
	  for (scenario in c("Class", "Gen")) {
	    WorkingDir = paste0("/home/v1jobste/JanaO/10K/",strategy, "_permEnv/", scenario, rep, "_", repeats, "/SimulatedData/")
...
write.table(TGVsAll, paste0("TGVsAll_permEnv_", strategy, "_", date, ".csv"), quote=FALSE, row.names=FALSE)
