...
#REQUIRED INPUT
#|||||||||||||||||||||||||||||||||||||||||||||||||
DATEDOWNLOAD=07072016 #date of downloading the file
...
do
	...
	cp $GEN_PATH/PARAMFILE.txt . #copy PARAMFILE for Zanardi to the directory and edit it for adding new files to the MERGED FILE
...
cp $GEN_PATH/NewIndForGovedo.R $TEMPDIR
...
cd $TEMPDIR/OUTPUT
cp $GEN_PATH/Add_alleles_Govedo_PARENTAL_SNP800.R .
