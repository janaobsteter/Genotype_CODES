...
for (strategy in c("SU55", "SU51", "SU15")) {
  ...
  for (scenario in c("Class", "GenSLO", "OtherCowsGen", "BmGen", "Gen")) {
    for (rep in 0:19)
      ...
      # inb <- as.matrix(read_table("/home/jana/bin/AlphaRelate/example/Example1Pedigree/Inbreeding_Class1.txt",  col_names=FALSE,  col_types = cols(X1 = col_character(), .default = col_double())))
