...
#REQUIRED INPUT
#|||||||||||||||||||||||||||||||||||||||||||||||||
DATEDOWNLOAD=15112016 #date of downloading the file
FILE=Matija_Rigler_BOVUHDV03_20161102.zip # downloaded file
#|||||||||||||||||||||||||||||||||||||||||||||||||
...
do
	...
	sed -i "s%SI %SI%g" ${_FINREP} #remove blank space from SI #####
	sed -i "s%SI %SI%g" ${_FINREP} #remove blank space from SI #####
	sed -i "s%  BLONS% BLONS%g" ${_FINREP} #remove blank space from SI #####
