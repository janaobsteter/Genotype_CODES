...
SU55_Rep3 <- read.table("~/Genotipi/Genotipi_CODES//TGVsAll_10KRef_2501.csv", header=TRUE) #če imaš 60 generacij, je standardizirano na 1. generacijo!!!
SU55_Rep3 <- SU55_Rep3[(SU55_Rep3$Rep %in% 0:2) & (SU55_Rep3$scenario=="Gen"),]
...
colnames(SU55_Rep3)[10] <- "Degree"
TGVsAll <- rbind(TGVsAll, SU55_Rep3)
...
st40 <- data.frame()
for (rep in 0:2) {
  for (degree in c(15, 30, 45, 60, 75, "minCoan", "maxGain", "Gen")) { #, "SU15", "SU51"
...
TGVsAll$Group <- as.factor(TGVsAll$Group)

TGVsAll <- TGVsAll[!(TGVsAll$Degree %in% c("minCoan", "maxGain")),]
maxmin <- maxmin[!(maxmin$degree %in% c("minCoan", "maxGain")),]
#o je z na genetsko standadrizirano gensko variacno
Year1Eff <- ggplot(data = TGVsAll, aes(x=GENICSd_St, y=MeanGENIC_genetic, group=Group, colour=Degree, linetype=Degree)) + 
  scale_x_reverse(sec.axis=sec_axis(trans=~1-.,                                   
                                    name="Pretvorjena / izgubljena genska standardna deviacija")) +
  geom_line(size=0.5, alpha=0.4) + 
  xlab("Generacija") + ylab("Prava genetska vrednost")  + ylim(0,7) +coord_cartesian(xlim = c(1, 0.75)) +
  scale_linetype_manual(breaks = c(15, 30, 45, 60, 75, "Gen"), # "minCoan", "maxGain",
                        "Scenarij", 
                        values=c("solid", "dotted","dashed", "dotdash", "twodash", "solid"), 
                        labels=c("OCS15", "OCS30", "OCS45", "OCS60", "OCS75",  "SLO_selekcija")) + 
  scale_colour_manual(breaks = c(15, 30, 45, 60, 75,   "Gen"), 
                      "Scenarij", 
                      values=c("forestgreen", "dodgerblue2", "purple", "red3", "orange1",  "black"), 
                      labels=c("OCS15", "OCS30", "OCS45", "OCS60", "OCS75",  "SLO_selekcija")) + 
  xlab("Genska standardna deviacija") + ylab("Povprečna prava genetska vrednost") + 
  ...
  geom_segment(data=maxmin, mapping=aes(x=maxGenicSD, xend=minGenicSD,
                                        y=minTGV,  yend=maxTGV,                                    
                                        color=degree, linetype=degree, group=degree), arrow=arrow(), show.legend=TRUE, size=1.5)
#to je samo optiSel 
...
ggplot(data = TGVsAll, aes(x=SDSt, y=zMean, group=Group, colour=Degree, linetype=Degree)) + 
...
genetic <- ggplot(data = MeanGenic, aes(x=Generation, y=GENICSd_St, group=Group, colour=Degree, linetype=Degree)) +  
  geom_line(aes(linetype=Degree), size=0.5, alpha=0.3) + 
  scale_linetype_manual(breaks = c(15, 30, 45, 60, 75, "Gen"), # "minCoan", "maxGain",
                        "Scenarij", 
                        values=c("solid", "dotted","dashed", "dotdash", "twodash", "solid"), 
                        labels=c("OCS15", "OCS30", "OCS45", "OCS60", "OCS75",  "SLO_selekcija")) + 
  ...
  geom_segment(data=maxmin, mapping=aes(x=40, xend=60,
                                        y=maxGenicSD,  yend=minGenicSD,                                    
...
genic <- ggplot(data = TGVsAll, aes(x=Generation, y=zSdGenic, group=Group, colour=Degree, linetype=Degree)) +  geom_line(aes(linetype=Degree), size=1) + 
  xlab("Generation") + ylab("True genetic value")  + 
  scale_linetype_manual(breaks = c(15, 30, 45, 60, 75, "Gen"), 
                        "Degree", 
                        values=c("solid", "dotted","dashed", "dotdash", "solid"), 
                        labels=c(15, 30, 45, 60, 75,  "minCoan", "maxGain")) + 
  scale_colour_manual(breaks = c(15, 30, 45, 60, 75,  "minCoan", "maxGain"), 
                      "Degree", 
                      values=c("forestgreen", "dodgerblue2", "purple", "red3", "orange1", "grey", "black"), 
                      labels=c(15, 30, 45, 60, 75,  "minCoan", "maxGain")) + 
...
MeanGenic <- aggregate(TGVsAll$SDGenicSt ~ TGVsAll$Degree + TGVsAll$Generation, FUN="mean")
colnames(MeanGenic) <- c("Degree", "Generation", "SDGenic")
#genetic gain
library(nlme)
mean <- lmList(MeanTGV ~ Generation | degree, data=MeanAverage)
mean <- lmList(SDGenic ~ Generation | Degree, data=MeanGenic)
...
library(nlme)
regRep <- data.frame(Rep=NA, Intercept=NA, Slope=NA, Degree=NA)
for (sc in c(15, 30, 45, 60, 75, "Gen")) {
  df <- TGVsAll[TGVsAll$Degree==sc,]
...
avgSlope <- aggregate(regRep$Slope ~ regRep$Degree, FUN="mean")
avgInt <- aggregate(regRep$Intercept ~ regRep$Degree, FUN="mean")
...
genGainPlot <- ggplot() + 
  ...
  scale_colour_manual(breaks = c(15, 30, 45, 60, 75,  "Gen"), 
                      ...
                      values=c("forestgreen", "dodgerblue2", "purple", "red3", "orange1", "black"), 
                      labels=c("OCS15", "OCS30", "OCS45", "OCS60", "OCS75",  "SLO_selekcija")) + 
  ylab("Povprečna prava genetska vrednost") +
...
ggplot() + 
  ...
  scale_colour_manual(breaks = c(15, 30, 45, 60, 75,  "Gen"), 
                      ...
                      values=c("forestgreen", "dodgerblue2", "purple", "red3", "orange1", "black"), 
                      labels=c("OCS15", "OCS30", "OCS45", "OCS60", "OCS75",  "SLO_selekcija")) + 
  ylab("Povprečna genska varianca") +
  geom_line(data = MeanGenic, aes(x=Generation, y=SDGenic, colour=Degree, linetype=Degree), size=1.2) + 
