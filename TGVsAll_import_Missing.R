	...
	if (file.exists(paste0(WorkingDir,'/PedigreeAndGeneticValues.txt'))) {
		...
		for (group in c("home", "import")) {
		  for (trait in c(trait1, trait2)) {
			  homeMean30 <- TGVsum$value[TGVsum$Group == "home" & TGVsum$variable == paste0('gvNormUnres', trait) & TGVsum$Generation == 30]
			  homeSdGenetic30 <- TGVsum$sd[TGVsum$Group == "home" & TGVsum$variable == paste0('gvNormUnres', trait) & TGVsum$Generation == 30]
			  homeSdGenic30 <- sqrt(Var$GenVar[Var$Group == "home" & Var$Trait == trait & Var$Gen == 31])
		    ...
		    TGVGroup$SDSt <- TGVGroup$sd / TGVGroup$sd[1]
			  TGVGroup$SDSt30 <- TGVGroup$sd / homeSdGenetic30

		    #standardise genetic values with genetic standard deviation
		    TGVGroup$zMean <- (TGVGroup$value - TGVGroup$value[1]) / TGVGroup$sd[1]
		    TGVGroup$zMean_noC <- TGVGroup$value / TGVGroup$sd[1]
			  TGVGroup$zMean30 <- (TGVGroup$value - homeMean30) / homeSdGenetic30
		    ...
		    TGVGroup$SDGenicSt <- TGVGroup$SDGenic / TGVGroup$SDGenic[1]
	      	    TGVGroup$SDGenicSt31 <- TGVGroup$SDGenic / homeSdGenic30
...
write.table(TGVsAll, paste0("TGVsAll_import_", date, ".csv"), quote=FALSE, row.names=FALSE)
