...
phenoA$scenario <- factor(phenoA$scenario, levels = c("$G:$P = 2:1", "$G:$P = 1:1", "$G:$P = 1:2"))
...
ggplot(data=phenoA[phenoA$Ref  == "False",], aes(x=Generation, y=zMean, group=NoControls, colour=NoControls, linetype = BV)) +
  geom_line(size = 1) + 
  theme_bw(base_size=18, base_family="sans")  + 
  theme(legend.position="top", legend.text=element_text(size=16), legend.title=element_text(size=16),
        axis.text = element_text(size = 16)) + 
...
ggplot(data=accPlot[accPlot$Ref == "False",], aes(x=Gen, y=Cor, group=AgeCat, colour=AgeCat)) + 
  geom_line(size = 1)  + 
  theme_bw(base_size=18, base_family="sans")  + 
  theme(legend.position="top", legend.text=element_text(size=16), legend.title=element_text(size=16),
        axis.text = element_text(size = 16)) + 
...
ggplot(data=refsizeM, aes(x=Generation, y=value, group=NoControls, colour=NoControls)) +
  ...
  theme_bw(base_size=18, base_family="sans")  + 
  theme(legend.position="top", legend.text=element_text(size=16), legend.title=element_text(size=16),
        axis.text = element_text(size = 16)) + xlab("Year") + 
  scale_colour_manual("Number of recordings / lactation", 
                      values = c("#7dcbf5", "#62bff0", "#41b4f0", "#17a3eb", "#076ea3", "#024263", "black")) + 
  scale_linetype_manual("", 
                        values = c("dashed", "solid")) + 
